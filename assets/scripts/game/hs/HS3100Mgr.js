...
cc.Class({
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_begin_push',function(data){
            ...
            self._gamestate = 2;
            //上局是否荒庄
            self._lastLiuJu = data.lastLiuJu;
            //荒庄座位号
            self._lastZhuang = data.lastZhuang;
        ...
        this.node.on('guohu',function(data){
         ...
         //显示过胡
         this.node.on('guopeng',function(data){
            data = data.data;
            self.doGuoPeng(data);
        });
    ...
    doGuoPeng:function(data){
        this.dispatchEvent('guopeng_game',data);
    },

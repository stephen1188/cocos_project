cc.Class({
    ...
    onLoad () {
        var const_ddz = require("DDZ1803Const");
        cc.vv.game = this;
        cc.vv.game.config = {
            type:"ddz",
    ...
    start(){
        ...
        //回放
        var ReplayMgr = require("DDZ1803ReplayMgr");
    ...
    initEventHandlers:function(){
        ...
        this.node.on("begin",function(data){
         ...
         this.node.on("fapai",function(data){
            ...
            self.handlist = pai;
            self.begin();
            self.fapai(pai,data.data.luckpai,data.data.luckpeople);
        }),
        ...
        this.node.on('luck',function(data){
            self.luckpeople = data.data.luckpeople;
            self.luckpai = data.data.luckpai;
            //self.luckQiang(self.handlist,luckpeople,luckpai);
        });
    ...
    //该谁先抢地主
    luckQiang:function(list,luckpai,luckpeople,index){
        var self = this;
        var viewid = cc.vv.roomMgr.viewChairID(luckpeople);
        var mgr = this.node.getChildByName('mgr');
        var mark = cc.instantiate(this.pokerPrefab);
        var spriteFrame = cc.vv.utils.getPokerSpriteFrame(this.poker1Atlas,255);
        mark.getChildByName('card').getComponent(cc.Sprite).spriteFrame = spriteFrame;
        mgr.addChild(mark);
        mark.y = 182;
        var callback = null;
        ...
        switch(viewid){
            case 0:{
                endpos.x = -570 + index*70 + 80;
                endpos.y = -238;
                scale = 1;
                cc.vv.log3.info('飞入位置1:' + endpos.x);
                //list.splice(index,0,luckpai);
                callback = function(){
                    list.splice(index,0,luckpai);
                    self.nodeCard.removeAllChildren();
                    for(var i = 0; i < list.length; i++){
                        ...
                        card.x = -570 + i*60;
                        if(list[i] != 0){
                            var spriteFrame = cc.vv.utils.getPokerSpriteFrame(self.poker1Atlas,list[i]);
                            card.getChildByName('card').getComponent(cc.Sprite).spriteFrame = spriteFrame; 
                            card.getComponent('DDZPoker').setValue(list[i]);
                            card.value = list[i];
                            if(list[i] == luckpai){
                                self.selectCard(card.getComponent('DDZPoker'),1);
                                card.y = 45;
                            }
                            if(i == list.length-1){
                    ...
                    for(var i = 0; i < self.nodeCard.childrenCount; i++){
                        var node = self.nodeCard.children[i];
                        if(node.value == luckpai){
                            cc.vv.log3.info('飞入位置2:' + i + ',' + node.x);
                            break;
                        }
                    }
                    // var node = cc.instantiate(self.pokerPrefab);
                    // node.myTag = index;
                    // self.nodeCard.addChild(node);
                    // node.x = -570 + index*60;
                    // var spriteFrame = cc.vv.utils.getPokerSpriteFrame(self.poker1Atlas,luckpai);
                    // node.getChildByName('card').getComponent(cc.Sprite).spriteFrame = spriteFrame; 
                    // node.getComponent('DDZPoker').setValue(luckpai);
                    // node.value = luckpai;
                    // self.refreshHandCards();
                }
            }
            break;
            case 1:{
                endpos.x = this.othercards.children[0].x;
                endpos.y = this.othercards.children[0].y;
            }
            break;
            case 2:{
                endpos.x = this.othercards.children[1].x;
                endpos.y = this.othercards.children[1].y;
            }
            break; 
        }
        mark.runAction(
    ...
    //发牌
    fapai:function(pai,luckpai,luckpeople){
        var key = -1;
        if(cc.vv.roomMgr.stage == null && !cc.vv.roomMgr.is_replay && !this.isDingZhuang || this.no_rob){
        ...
        this.deal_fapai(0,pai.length,pai,function(idx){
            ...
            self.luckQiang(pai,luckpai,luckpeople,key);
            self.initKoupai();
            if(self.jipai){
    ...
    deal_fapai:function(begin,end,list,callback){
        ...
        //发牌
        var xipai = 0;
        if(cc.vv.roomMgr.param){
            xipai = cc.vv.roomMgr.param.xipai;
        }
    ...
    //显示出左上角三张底牌
    showDipai:function(pai,dizhu){
        //if(cc.vv.roomMgr.seatid != dizhu && !cc.vv.roomMgr.is_replay) return;
        for(var k = 0; k < 4; ++k){
            var item = this.dipai.children[k];
            item.active = true;
            var sprite = item.getComponent(cc.Sprite);
            if(cc.vv.roomMgr.seatid == dizhu || cc.vv.roomMgr.is_replay){
    ...
    moveKoupai:function(pai){
        ...
        for(var i = 0; i < this.koupai.childrenCount; ++i){
            ...
            node.runAction(cc.sequence(
                ...
                cc.callFunc(function(){
                    self.showDipai(pai,self._dizhu);
                }),
                cc.hide(),
                cc.removeSelf()
            ));
    ...
    dizhuRunAction2:function(seatid){
        //地主确定动画 
        var index = cc.vv.roomMgr.viewChairID(seatid);
        this.dizhu.active = true;
        var endposx = this.dizhupos.children[index].x;
        var endposy = this.dizhupos.children[index].y;
        this.dizhu.x = endposx;
        this.dizhu.y = endposy;
   },
    ...
    dingzhuang:function(data){
        ...
        if(cc.vv.roomMgr.is_replay){
            this.showDipai(data.dipai,data.seatid);
        }
        //翻转底牌
    ...
    stage:function(data){
        ...
        switch(data.stage){
            ...
            case 3:{
                ...
                this.updateBeiShu(data.power,data.nowZha);
                this.dizhuRunAction2(data.zhuang);
                this.showDipai(data.dizhupai,data.zhuang);
                for(var i = 0; i < data.pais.length; ++i){
                    this.showPais(i,data.pais[i]);
                }
            ...
            case 5:{
                ...
                for(var i = 0; i < data.openPai.length; ++i){
                    if(data.openPai[i] == 1){
                        this.isMingPai = true;
                        this.showMingPai(data.pai[data.zhuang],data.zhuang);
                    }
                }
            ...
            case 7:{
                ...
                for(var i = 0; i < 3; ++i){
                    var index = cc.vv.roomMgr.viewChairID(i);
                    this.showPais(index,data.pais[i]);
                }

cc.Class({
    ...
    onLoad () {
        var const_ddz = require("DDZ1800Const");
        cc.vv.game = this;
        cc.vv.game.config = {
            type:"ddz",
    ...
    start(){
        ...
        //回放
        var ReplayMgr = require("DDZ1800ReplayMgr");
    ...
    jiesuan:function(data){
        ...
        for(var i = 0; i < 3; ++i){
            ...
            for(var n = results[i].outHand.length - 1 ; n >= 0; --n){
                cc.vv.log3.info(n+'：：一共有多少张出牌：：'+results[i].outHand.length)
                cc.vv.log3.info("界面上除了多少张："+chupai.children[n].length)

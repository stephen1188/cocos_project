    ...
    getNextAction:function(){
        ...
        if(this._currentIndex >= this._actionRecords.length){
            ...
            if(this._jiesuan != null && this._currentIndex > 0){
                cc.vv.TTZMgr.node.emit("jiesuan",{data:this._jiesuan});
            }
    ...
    takeAction:function(){
        ...
        if(action.action == ACTION_SCORE){
            ...
            for(var i = 0; i < list.length; ++i){
                ...
                cc.vv.TTZMgr.node.getComponent('Table').seat_emit(viewid,'score',action.data[i].score);
            }
            return 0.2;
        }else if(action.action == ACTION_INIT){
            cc.vv.TTZMgr.doInit(action.data);
            return 0.5;
        }else if(action.action==ACTION_QIANG){
            cc.vv.TTZMgr.doQiang(action.data);
            return 2;
        }else if(action.action ==  ACTION_DINGZHUANG){
            cc.vv.TTZMgr.doDingZhuang(action.data);
            return 1;
        }else if(action.action == ACTION_GETEND){
            cc.vv.TTZMgr.doGetEnd(action.data);
            return 3;
        }else if(action.action == ACTION_XIAZHU){
            cc.vv.TTZMgr.doXiaZhu(action.data);
            return 1;
        }else if(action.action == ACTION_KAIPAI){
            cc.vv.TTZMgr.doKaipai(action);
            return 1;
        }
    }
});

var DAO_POS = [cc.v2(-410,304),cc.v2(-118,304),cc.v2(250,304)];
var DAO_ADD = [75,75,75];
var DAO_MAX = [2,3,3];
...
cc.Class({
    ...
    initBaiPaiListX:function(){
        ...
        daoList[0].x = -((daoList.length - 1)*84)/2;
        daoList[0].zIndex = 0;
        for(var i = 1 ; i < daoList.length;i++){
            daoList[i].x = daoList[i-1].x+ 84;
            daoList[i].zIndex = i;
        }
    ...
    checkOne:function(){
        ...
        if(daoList.length > 3 || daoList.length == 0)return false;

        //检查头道是否为空
        var dao = this.getDaoNullIndex(0);
        if(dao == 0){
            var cardData = [];
            for(var j = 0;j < daoList.length;j++){
                card =  daoList[j].getComponent('SSZBaiPaiCard');
                cardData.push(card.getPai());
            }

            if(cardData.length == 2){
                card = daoList[0].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
                card = daoList[1].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
            ...
            if(cardData[0] == 0x42 ||　cardData[0] == 0x43){
                card = daoList[0].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
                
                if(cardData[1] %16 >= cardData[2] %16){
                    card = daoList[1].getComponent('SSZBaiPaiCard');
                    this.selectCard(card,1);
                }
                else{
                    card = daoList[2].getComponent('SSZBaiPaiCard');
                    this.selectCard(card,1);
                }
            ...
            else if(cardData[0] %16 == cardData[1] %16){
                card = daoList[0].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
                card = daoList[1].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
                this.onDaoClick(null,0);
                return true;
            }else if(cardData[1] %16 == cardData[2] %16){
                card = daoList[1].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
                card = daoList[2].getComponent('SSZBaiPaiCard');
                this.selectCard(card,1);
    ...
    onDaoClick:function(event,data){
        ...
        //判断牌是否满足每道的最大牌数
        if(DAO_MAX[dao] != selectList.length){
            return;
        }
        ...
        for(var i= 0 ;i < selectList.length;i++){
            if(j > 0){
                ...
                selectList[i].zIndex = dao * 10 + index;
    ...
    getSelectPaiList:function(selected){
        var result = [];
        var children = this.nodeHandList.children;
        for (var i = 0; i < children.length; ++i) {
            var card =  children[i].getComponent('SSZBaiPaiCard');
    ...
    updatePaiBtn:function(){
        ...
        this.scheduleOnce(function(){
            ...
            //根据手上的牌，算出可以组出的牌型
            that.paiListType = [0,0,0,0];

            if(paiList.length > 2){
                //取尾道
                that.sszGame.sszCardList.initData();
                that.sszGame.sszCardList.getCardTypeAi(paiList,that.paiListType,9999,(paiList.length>2)?1:3,0,true);

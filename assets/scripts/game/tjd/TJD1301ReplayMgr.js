    ...
    getNextAction:function(){
        ...
        if(this._currentIndex >= this._actionRecords.length){
            ...
            if(this._jiesuan != null && this._currentIndex > 0){
                cc.vv.TJDMgr.node.emit("jiesuan",{data:this._jiesuan});
            }
    ...
    takeAction:function(){
        ...
        if(action.action == ACTION_SCORE){
            ...
            for(var i = 0; i < list.length; ++i){
                ...
                cc.vv.TJDMgr.node.getComponent('Table').seat_emit(viewid,'score',action.data[i].score);
            }
            return 0.2;
        }else if(action.action == ACTION_INIT){
            cc.vv.TJDMgr.doInit(action.data);
            return 0.5;
        }else if(action.action==ACTION_QIANG){
            cc.vv.TJDMgr.doQiang(action.data);
            return 2;
        }else if(action.action ==  ACTION_DINGZHUANG){
            cc.vv.TJDMgr.doDingZhuang(action.data);
            return 1;
        }else if(action.action == ACTION_GETEND){
            cc.vv.TJDMgr.doGetEnd(action.data);
            return 3;
        }else if(action.action == ACTION_XIAZHU){
            cc.vv.TJDMgr.doXiaZhu(action.data);
            return 1;
        }else if(action.action == ACTION_KAIPAI){
            cc.vv.TJDMgr.doKaipai(action.data);

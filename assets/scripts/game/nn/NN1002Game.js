...
cc.Class({
    ...
    onLoad(){
        ...
        //牛牛配置
        var const_nn = require("NN1002Const");
        cc.vv.game = this;
        cc.vv.game.config = {
            type:"nn",
    ...
    start(){
        ...
        // //回放
        var ReplayMgr = require("NN1002ReplayMgr");
    ...
    fapai_1:function(data){
        ...
        //发牌
        this.deal_fapai(0,5,data,function(viewid,idx){
            //另外最后一张，不要翻
    ...
    beginXiaZhu:function(data){
        ...
        var callback = function(){
            var viewid = cc.vv.roomMgr.viewChairID(data.seatid);
            self.table.seat_emit(null,"beginXiaZhu",{seatid:viewid});
            if(cc.vv.roomMgr.is_replay != true){
                    //自己不是庄家
	...
	isHaveNiu : function (array) {
        ...
        for(var i = 0;i < byTestPai.length;i++){
            if(byTestPai[i] == 95 || byTestPai[i] == 79){
                card_lazi[laizicount] = byTestPai[i];
                laizicount++;
            }else{
        ...
        if(laizicount == 1){
            ...
            if(((card_i % 16) + (card_j % 16)) % 10 == 0 && is_no_wang_maxmin != -1){
                for(var i = 0;i<byTestPai.length;i++){
                    if(byTestPai[i] == 95 || byTestPai[i] == 79){
                        continue;
                    }

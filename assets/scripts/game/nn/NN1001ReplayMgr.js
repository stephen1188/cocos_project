    ...
    getNextAction:function(){
        ...
        if(this._currentIndex >= this._actionRecords.length){
            ...
            if(this._jiesuan != null && this._currentIndex > 0){
                cc.vv.jgnnMgr.node.emit("jiesuan",{data:this._jiesuan});
            }
    ...
    takeAction:function(){
        ...
        if(action.action == ACTION_SCORE){
            ...
            for(var i = 0; i < list.length; ++i){
                ...
                cc.vv.jgnnMgr.node.getComponent('Table').seat_emit(viewid,'score',action.data[i].score);
            }
            return 0.5;
        }else if(action.action == ACTION_INIT){
            cc.vv.jgnnMgr.doInit(action.data);

            return 0.5;
        }else if(action.action == ACTION_DINGZHUANG){
            cc.vv.jgnnMgr.doDingZhuang(action.data);
            return 2;
        }else if(action.action == ACTION_FAPAI){
            cc.vv.jgnnMgr.doFapai(action.data);

            return 3;
        }
        else if(action.action == ACTION_XIAZHU){
            cc.vv.jgnnMgr.doXiaZhu(action.data);

            return 2;
        }else if(action.action == ACTION_KAIPAI){
            cc.vv.jgnnMgr.doKaipai(action.data);

...
cc.Class({
    ...
    getNextAction:function(){
        ...
        if(data.action != null){
            return {data:data.data,action:data.action,pubPoker:data.pubPoker};
        }
    ...
    takeAction:function(){
        ...
        if(action.action == ACTION_SCORE){
            ...
            for(var i = 0;i < this._actionRecords.length;i++){
                var data = this._actionRecords[i];
                if(data.action == "hand"){
            ...
            for(var i = 0;i < action.data.length;i++){
                for(var j = 0;j < action.data.length;j++){
                    if(action.data[i][j] == this.pubPoker && j != 4){
                        var a = action.data[i][4];
                        action.data[i][4] = action.data[i][j];
                        action.data[i][j] = a;
                    }
                }
            }
            ...
            cc.vv.nnMgr.doKaipai(action.data,action.pubPoker);
            return 2;
        }
    }
});

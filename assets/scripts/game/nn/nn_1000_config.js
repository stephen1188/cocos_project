cc.Class({
    ...
    onLoad: function () {
        ...
        this.tuiZhu = this.node.getChildByName('tuiZhu').getChildByName("bg").getChildByName("difennum").getComponent(cc.Label);//推注倍数
    ...
    loadSaveConfig:function(){
        ...
        //大众牛牛
        var conf = cc.sys.localStorage.getItem("last_game_nn_1000");
        if(conf){
            ...
            if(json.zhuang != "qiang"){
                if(this.node.getChildByName('zhuang').getChildByName("qiang").getComponent(cc.Toggle).isChecked){
                    this.is_limit_QZ = true;
                }
            ...
            if(json.tuiZhu){
                this.tuiZhu.string = json.tuiZhu;
                this._tuizhu = json.tuiZhu;
                if(parseInt(json.tuiZhu) > 0){
                    if(json.overflow){
                        cc.vv.utils.setToggleChecked(this.node.getChildByName('overflow'),0);
                    }
                }
            }
    ...
    //按钮事件
    onBtnClick_tuizhu:function(event, data){
        if(this.node.getChildByName('feng').active == true){
            return;
        }
        var self = this;
        switch(event.target.name){
            case "add":{
                this.tuizhu_index += 1;
                var length = this.tuizhuArr.length;
                var guodi = this.tuizhuArr[this.tuizhu_index % length];
                this.tuiZhu.string = guodi;
            }
            break;
            case "sub":{
                var length = this.tuizhuArr.length;
                this.tuizhu_index -= 1;
                if(this.tuizhu_index < 0){
                    this.tuizhu_index += length
                }
                var guodi = this.tuizhuArr[this.tuizhu_index % length];
                this.tuiZhu.string = guodi;
            }
        ...
        //不可负分、推注互斥
        // var tuizhu = parseInt(this.tuiZhu.string);
        // if(tuizhu > 0){
        //     cc.vv.utils.setToggleChecked(this.node.getChildByName('overflow'),0);
        // }
        // this._tuizhu = this.tuiZhu.string;
    },
    //按钮事件
   ...
   //托管时间 加减框
    ...
    //不可负分、推注互斥
    btn_overflow:function(event){
        // if(this.node.getChildByName('feng').active == true){
        //     return;
        // }
        // if(event.isChecked){
        //     this.tuiZhu.string = "0";
        // }else{
        //     this.tuiZhu.string = this._tuizhu;
        // }
    },
    ...
    Config:function(daikai){
        ...
        switch (ren) {
            case "5ren": {
               desc.push("5人场");
            //    this.node.getChildByName('ju').getChildByName("10ju").getChildByName("tips").getChildByName("num").getComponent(cc.Label).string = "2";
            //    this.node.getChildByName('ju').getChildByName("20ju").getChildByName("tips").getChildByName("num").getComponent(cc.Label).string = "4";
            }
            break;
            case "10ren": {
               desc.push("10人场");
            //    this.node.getChildByName('ju').getChildByName("10ju").getChildByName("tips").getChildByName("num").getComponent(cc.Label).string = "4";
            //    this.node.getChildByName('ju').getChildByName("20ju").getChildByName("tips").getChildByName("num").getComponent(cc.Label).string = "8";
            }
            break;
        }
        ...
        desc.push("推注"+ this.tuiZhu.string +"倍");

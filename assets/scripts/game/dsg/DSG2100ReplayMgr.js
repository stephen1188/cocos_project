...
cc.Class({
    ...
    getNextAction:function(){
        ...
        if(this._currentIndex >= this._actionRecords.length){
            ...
            if(this._jiesuan != null && this._currentIndex > 0){
                cc.vv.dsgMgr.node.emit("jiesuan",{data:this._jiesuan});
            }
    ...
    takeAction:function(){
        ...
        if(action.action == ACTION_FAPAI){
            cc.vv.dsgMgr.doFapai(action.data);

            return 3;
        }else if(action.action == ACTION_DINGZHUANG){
            cc.vv.dsgMgr.doDingZhuang(action.data);

            return 1;
        }
        else if(action.action == ACTION_XIAZHU){
            cc.vv.dsgMgr.doXiaZhu(action.data);

            return 2;
        }else if(action.action == ACTION_KAIPAI){
            cc.vv.dsgMgr.doKaipai(action.data);

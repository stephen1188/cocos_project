...
cc.Class({
    ...
    onLoad(){
        ...
        //牛牛配置
        var const_nn = require("DSG2101Const");
        cc.vv.game = this;
        cc.vv.game.config={
            type:"jgdsg",
            ...
            player_10:const_nn.player8,
            cuopai:const_nn.cuopai,
            selfPoke_5:const_nn.selfPoke5,
            selfPoke_10:const_nn.selfPoke8,
            set_bg:false,
            location:false,
    ...
    start () {
        ...
        // //回放
        var ReplayMgr = require("DSG2101ReplayMgr");
    ...
    beginXiazhu:function(data){
        this.show_Xiazhu = [];
        if(data.minXiazhu != null)
            this.show_Xiazhu[0] = data.minXiazhu;
        else
            this.show_Xiazhu[0] = this.show_min_Xiazhu
        if(data.maxXiazhu != null)
            this.show_Xiazhu[1] = data.maxXiazhu;
        else
            this.show_Xiazhu[1] = this.show_max_Xiazhu
        this.show_xiazhu();
    },
    //监听协议
    ...
    fapai_1:function(data, isXiazhu){
        ...
        //发牌
        this.deal_fapai(0,3,data,isXiazhu,function(viewid,idx){
    ...
    fapai_1_stage:function(data, isXiazhu){
        ...
        //发牌
        this.show_fapai(0,3,data,isXiazhu,function(viewid,idx){
    ...
    show_xiazhu(isStage){
        ...
        if(this._feng != 0){
            ...
            this.tip("xiazhu");
            if(this.show_Xiazhu != null)
                var small = this.show_Xiazhu[0];
            else
                var small = this.show_min_Xiazhu;
            if(this.show_Xiazhu != null)
                var max = this.show_Xiazhu[1];
            else
                var max = this.show_max_Xiazhu;
   ...
   kaipai_callback:function(obj,viewid,idx,type,rate,pai){
    ...
    nodebg.zIndex = 100;
    nodebg.scale = scale;
    nodebg.x = card_pos.x + toright - 30;
    nodebg.y = card_pos.y - 40;
    if(viewid == 0){
        nodebg.y = card_pos.y - 60;
        nodebg.x = card_pos.x + toright - 28;
    }
    if(pai != null){
        var arrTop = [];
        var isYidong = false;
        if(type != null &&type%10 == 5){
    ...
    kaipai:function(data){
        ...
        var type = data.type;
        //音效
        this.play_dsg_mp3(data.seatid,type);
    ...
    jiesuan:function(data){
        ...
        if(!this.isStage || cc.vv.roomMgr.stage.stage != 97){
            //所有人开牌
            for(var i=0;i< cc.vv.roomMgr.real ;++i){
                var viewid = cc.vv.roomMgr.viewChairID(i);
        ...
        for(var i=0;i< cc.vv.roomMgr.real ;++i){
            ...
            if(viewid != 0 && isTrue == true){
                ...
                var url =  "dsg_" + (data.list[i].type - 1);
    ...
    stage:function(data){
        ...
        switch(data.stage){
            ...
            break;
            case 2:{
                this.show_max_Xiazhu = data.max;
                this.show_min_Xiazhu = data.min;
    ...
    play_dsg_mp3(setaid,type){
        ...
        var mp3File = "dsg/" + sex + "/type_" + (type - 1);
        cc.vv.audioMgr.playSFX(mp3File);
    },
    //给所有牌发消息

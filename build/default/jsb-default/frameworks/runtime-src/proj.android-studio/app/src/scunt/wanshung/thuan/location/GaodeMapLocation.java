package scunt.wanshung.thuan.location;
...
public class GaodeMapLocation {
    ...
    private void Callback(AMapLocation loc){
        //写一个游戏是否已经启用的状态
        AppActivity.GameStartd = true;
        if(AppActivity.yq_roomid!="" && AppActivity.yq_roomid !=""){
            final String roomId = AppActivity.yq_roomid;
            AppActivity.yq_roomid = "";
            G3Plugin.getActivity().runOnGLThread(new Runnable() {
                @Override
                public void run() {
                    String eval = String.format("cc.vv.g3Plugin.onXianliaoYQ(%d,\"%s\",\"%s\",\"%s\");",1,roomId,roomId,roomId);
